...
contract BinanceAtomicSwap {
    ...
    struct Swap {
        ...
        uint256 starttime; // starttime timestamp of the swap
        uint256 timeLockDuration; // Duration of the time-lock period
        uint256 amount; // Amount to be swapped
        bytes32 hashlock; // Hash of the secret
        // int8 executed; // Flag to track if the swap has been executed
        // int8 refunded; // Flag to track if the swap has been refunded
    }
    ...
    function checkHash(uint256 swapID, bytes32 secretHash) external view returns (bool) {
        ...
        return (swap.initiator != address(0) && swap.hashlock == secretHash);
    }
